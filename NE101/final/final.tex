...
\item Shell model comes into play here. Normally, a single energy
  ...
  \end{figure}
\item What does this have to do with fission? Well, as the nucleus
  becomes more deformed ($\epsilon\uparrow$), some nucleon energies go
  up and some go down (as discussed above). If the nucleus is not
  stable under fission, its binding energy is going up like
  $\epsilon^2$ as it runs towards fission. If the valence nucleons are
  in a state where their
  $E\uparrow$ as $\epsilon\uparrow$ (above the dotted line in
  Figure~\ref{fig:levelsplit}), the total energy goes up even
  faster.

  \vspace{5pt}
  Woohoo fission here we come.

  \vspace{5pt}
  This doesn't last forever. Eventually, as $E\uparrow$ and
  $\epsilon\uparrow$, there will be a lower-energy state for those
  valence nucleons where $E\downarrow$ as $\epsilon\uparrow$ (below
  the dotted line). This happens because the \textit{real} diagram is
  super complicated and messy, (it's Figure 5.29 in Krane if you want
  to look at it). Now, as we keep getting more deformed, the nucleons
  are acting \textit{against} fission, so we get an energy dip. This
  makes a little valley of stability and another hump we have to
  overcome to cause fission. This is shown in
  Figure~\ref{fig:fissionhumps}.~\cite[pp. 493-495]{krane}

  \begin{figure}[hbtp]
    \centering
    \includegraphics[scale=0.75]{images/shell_model}
...
\item $\eta$: The number of fast neutrons per \textit{thermal neutron absorbed}.
\end{itemize}
